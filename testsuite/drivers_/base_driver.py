...
from drivers_.valgrind import Valgrind
...
class BaseDriver(DiffTestDriver):
    ...
    def set_up(self):
        ...
        # if (
        #     self.env.options.skip_internal_tests and self.test_env['test_name'].startswith('internal__tests')
        # ):
    ...
    def check_file_list(self, what, file_list, can_be_empty=True):
        ...
        if (not isinstance(file_list, list) or (not can_be_empty and len(file_list) == 0) or not all(isinstance(fn, str) for fn in file_list)):
            empty_msg = 'non-empty '
            raise TestAbortWithError(
