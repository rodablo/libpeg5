...
from drivers_.base_driver import BaseDriver
...
class ParserDriver(BaseDriver):
    ...
    def test_unparsing(self):
        ...
        :rtype: bool
        """
        return self.test_env.get('test-unparsing', False)
    ...
    def get_lookups(self):
        ...
        for lookup in lookups:
            if (
                not isinstance(lookup, dict) or len(lookup) != 2 or not isinstance(lookup.get('line'), int) or not isinstance(lookup.get('column'), int)
            ):
